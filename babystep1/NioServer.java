...
public class NioServer {
	 ...
	 * 
	 * @param the
	 *            key of the channel on which the incoming data waits to be received
	 */
	private void handleRead(SelectionKey key) throws IOException {
		...
		sc.read(inBuffer);
		/*
		 * if (state == READ_LEN) { sc.read(lenBuffer);
		 * 
		 * if (lenBuffer.remaining() ==0) {
		 * 
		 * state = READ_PLOAD; lenBuffer.position(0); int len = lenBuffer.getInt();
		 * ploadBuffer = ByteBuffer.allocate(len); } } else if (state == READ_PLOAD)
		 */
	 ...
	 * 
	 * @param the
	 *            key of the channel on which data that should be sent
	 * @param the
